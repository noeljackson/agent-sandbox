...
When a `SandboxClaim` creates or adopts a `Sandbox`, agent-sandbox propagates the `agents.x-k8s.io/claim-uid` label to both the claimed `Sandbox` and its backing `Pod`. The UID is immutable and survives delete/recreate of identically-named claims.

Example:

```sh
kubectl get pods -A -l agents.x-k8s.io/claim-uid=<claim-uid>
```
